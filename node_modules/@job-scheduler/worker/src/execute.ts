...
import { assertTransition, computeBackoffMs } from "@job-scheduler/shared";
...
export async function executeJob(job: any) {
  ...
  try {
    assertTransition("CLAIMED", "RUNNING");
    ...
    if (attempts <= job.maxAttempts) {
      const delay = computeBackoffMs(queue?.retryPolicy?.strategy ?? "EXPONENTIAL", attempts, queue?.retryPolicy?.baseDelayMs ?? 1000, queue?.retryPolicy?.maxDelayMs ?? 10000);
      ...
      await prisma.deadLetterJob.create({ data: { jobId: job.id, reason: (error as Error).message } });
      await prisma.jobLog.create({ data: { jobId: job.id, level: "error", message: (error as Error).message } });
      console.log(`Worker ${workerId} - job ${job.id} moved to dead letter: ${(error as Error).message}`);
    }
  }
}
